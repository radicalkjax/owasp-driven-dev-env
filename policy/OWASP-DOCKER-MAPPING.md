...
| D04 | Secure container runtime | ✅ Yes | `devcontainer.json` runs `--cap-drop=ALL` then adds back only `NET_ADMIN`/`NET_RAW` — the two capabilities `init-firewall.sh` actually needs — instead of relying on Docker's broader default capability set | No seccomp/AppArmor profile is set, and the root filesystem isn't read-only (a full interactive dev environment needs broad write access, so this repo doesn't attempt that trade-off). |
| D05 | Secure the supply chain | ✅ Yes | Base image pinned by digest; the Nix installer and gitleaks are pinned to specific versions and checksum-verified before running (see `check-secrets-scan.sh` and the Dockerfile); `flake.lock` pins the rest of the dev toolchain (once generated — see `flake.nix`); `actions/checkout` in the CI gate is pinned by commit SHA, not a mutable tag | No SBOM is generated for the built devcontainer image itself. |
| D06 | Secure container registries | ❌ No | — | Registry access control, image signing/verification (e.g. cosign), and retention policy are properties of wherever this image gets pushed, not of this repo. |
| D07 | Process for handling relevant security patches | ⚠️ Partial | Every pinned version (base image, Nix, gitleaks, `actions/checkout`) is a concrete value that shows up in a diff when it changes, so a bump is visible and reviewable | There's no automated process (e.g. Dependabot/Renovate) opening those bump PRs — pins currently have to be refreshed by hand. |
...
| D09 | Secure Docker networking | ✅ Yes | `init-firewall.sh` enforces default-deny egress — the container can't reach anything beyond loopback, DNS to a configured resolver, and an explicit domain allowlist | Only egress is filtered. Ingress/inter-container networking isn't a concern here since this is a single-container dev sandbox, not a multi-service deployment. |
...
**Bottom line:** 4 of 10 are fully covered, 2 are partial, and 4 (host
hardening, daemon hardening, registry security, log collection) are
infrastructure-level decisions that belong to wherever this container is
actually deployed and run — not something a devcontainer definition in a
git repo can guarantee on its own.
