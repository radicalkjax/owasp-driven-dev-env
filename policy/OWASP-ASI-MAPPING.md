...
| ASI04 | Agentic Supply Chain | ✅ Yes | `check-aibom-updated.sh` fails a PR that touches dependency manifests, MCP server configs, or agent YAML files without also updating `aibom.json`, **and** `check-flake-lock.sh` enforces that the dev toolchain itself is hash-pinned via `flake.lock` (Nix) rather than resolved fresh per machine | `check-aibom-updated.sh` only enforces that an update happened, not that it's correct or complete — a real SCA / AI-inventory tool should replace the hand-maintained `aibom.json` in production. `flake.lock` gives exact, reproducible toolchain hashes but doesn't scan those pinned packages for known vulnerabilities — pinning and vulnerability scanning are different problems. |
| ASI05 | Unexpected Code Execution | ✅ Yes | `check-sandbox-integrity.sh` plus the default-deny egress firewall, packaged as the reusable `agentic-sandbox-firewall` Dev Container Feature (`src/agentic-sandbox-firewall/`) — even arbitrary agent-generated code can't exfiltrate data or reach an unapproved host. The firewall re-applies via the Feature's `entrypoint` on *every* container start, not just first creation — a stop/start cycle doesn't silently drop it. | The firewall resolves allowlisted domains to IPs once, per container start — see the limitation comment in `src/agentic-sandbox-firewall/install.sh`. CDN/rotating-IP domains may need the container restarted or an SNI-aware egress proxy. |
